...
void handle_parent(session_t *sess)
{
	...
	while(1){
		...
		case PRIV_SOCK_PASV_ACTIVE:
			...
			break;
		case PRIV_SOCK_GET_PASV_LISTEN:
			privop_pasv_listen(sess);
			break;
		case PRIV_SOCK_GET_PASV_ACCEPT:
...
{
	/*
	nobody进程接受PRIV_SOCK_GET_DATA_SOCK命令，
	进一步接收一个整数，也就是端口号port，接收一个字符串（ip）


	nobody创建套接字，fd = socket
	绑定20端口号，
	与客户端连接connect(ip, port)
	*/
