...
ssize_t readline(int sockfd, void *buf, size_t maxline) //封装的readline函数只能用于套接口，因为用recv函数来实现的
{
	int ret;
	int nread;// 接受到的字节数
	char *bufp = buf;
...
int read_timeout(int fd, unsigned int wait_seconds)
{
    ...
    if(wait_seconds > 0){
        ...
        timeout.tv_usec = 0; //由于这里是用秒记数因此不关心微秒
        do{
...
// deactivate_nonblock -设置为非阻塞模式

void deactivate_nonblock(int fd)
{
    int ret;
    int flags = fcntl(fd, F_GETFL); // fcntl可以改变打开的文件性质
                                    //F_GETFL 取得文件描述符状态旗标，此旗标为open（）的参数flags。
    if(flags == -1)
        ERR_EXIT("fcntl");

    flags &= ~O_NONBLOCK;
    ret = fcntl(fd, F_SETFL, flags); //F_SETFL 设置文件描述符状态旗标，参数arg为新旗标
...
int connect_timeout(int fd, struct sockaddr_in *addr, unsigned int wait_seconds)
{
    ...
    if(ret < 0 && errno == EINPROGRESS){
        ...
        if(ret == 0){
            ...
            // ret 返回1， 可能有两种情况， 一种是连接建立成功， 一种是套接字产生错误
            //错误的信息不回保存至errno变量中，因此需要调用getsockopt来获取
            int err;
